...
def read_fragments(filename, file_type="csv", smiles_col="smiles"):
    ...
    if file_type == "csv":
        ...
        lead_sets.append([Chem.MolFromSmiles(s) for s in smiles])
    else:
...
def read_config_json(json_dict):
    #remove scoring function section for now
    copy_dict=json_dict.copy()
    if "scoring_function_config" in copy_dict:
        del copy_dict["scoring_function_config"]
    #collapse nested dict into one level
    new_json = {}
    for key in copy_dict:
        if isinstance(copy_dict[key], dict):
            for subkey in copy_dict[key]:
                new_json[subkey] = copy_dict[key][subkey]
        else:
            new_json[key] = copy_dict[key]
    for key in new_json:
        if key not in gl.PARAMS.keys():
            print("Unknown parameter {} in configuration file".format(key))
        else:
            #update parameter
            gl.PARAMS[key] = new_json[key]
    ...
    for line in lines:
        ...
        if line.startswith('#') == False and len(info) > 0:         # comments start with #
            if info[0] not in gl.PARAMS.keys():
                ...
                gl.PARAMS[info[0]] = ast.literal_eval(info[2])
                #gl.PARAMS[info[0]] = eval(info[2])             # dictionary
            else:
