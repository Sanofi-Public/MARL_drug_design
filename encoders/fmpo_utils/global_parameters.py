...
_BBLOCKS_ROOT = os.environ.get("BBLOCKS_ROOT", "")
_RXN_FOLDER = os.environ.get("RXN_FOLDER", "")
_DEEPFMPO_SCORING_CONF = os.environ.get("DEEPFMPO_SCORING_CONF", "")
...
PARAMS = {# Input Files for Fragmentation
          ...
          "FRAGMENT_FILES": [f"{_BBLOCKS_ROOT}/BBlocks/*/*_R1.smi",     # name of file(s) with molecules or fragments for creating decodings
                             f"{_BBLOCKS_ROOT}/Scaffolds/*/*_R1.smi"],  # (only for GET_DECODINGS = 1 or 2)
          ...
          "RXN_FOLDER": _RXN_FOLDER, # folder with rxn files (only for SPLIT_OPTION = 2)
          
          # Scoring Function
          "SCORING_FUNCTION": "ScoringFunctionCalculator",  # scoring function to be used
          "SF_KWARGS": {"conf": _DEEPFMPO_SCORING_CONF},         # kwargs for scoring function
          
          # General stuff
          "MORE_OUTPUT": False,     # Do you want more output?
          "NUM_CORES": 30,           # cores used for parallelization of distance matrix (0 = non-parallel)
          "BATCHSIZE_D": 100000,   # batch size for parallelized distance matrix 
          ...
          "MAX_ATOMS": 20,               # maximum number of heavy atoms in fragment
          "MAX_FREE": 4,                 # maximum number of attachment points in fragment
          "SPLIT_OPTION": 1,             # 1 = SMARTS, 2 = RXN-Files
          #"SPLIT_SMARTS": ["[R]-&!@*"],  # splitting pattern (only for SPLIT_OPTION = 1)
          "SPLIT_SMARTS": ["[R]-&!@*", "[R]!@[!R]","N!@C!@C!@N"] ,
          #"N!@C!@C!@N"],  # splitting pattern (only for SPLIT_OPTION = 1)
          ...
          "FIX_BITS": 0,      # how many bits are fixed?
          ...
          # Sampling
          "NUM_SAMPLES": 100000,       # number of output molecules
          "SAMPLE_EPOCHS": None,    # samples are only taken from last ... epochs
          
          # for DeepVL
          "REACTIONFILE": "reactions_ugi.txt",                                           # reaction file
          "BBLOCKS": "/VL/BBlocks_source/20201125/BBlocks",                              # building block folder
          "LEAD_MOLECULES": ["ugi__HAG00317205__HAG00003558__HAG00013789__HAG00001637",  # lead molecules
                             "ugi__HAG00317205__HAG00003558__HAG00005874__HAG00001637"],
