...
from encoders.fmpo_utils.file_io import read_molfile
from encoders.fmpo_utils.file_io import create_folder
from encoders.fmpo_utils.file_io import read_fragments
from encoders.fmpo_utils.visualization import mols_to_svg
from encoders.fmpo_utils.similarity import similarity
from encoders.fmpo_utils.tree import build_tree_from_list
from encoders.fmpo_utils.fast_similarity import fast_similarity_with_prescreen
...
def create_decodings(lead_frags):
    ...
    for pattern in gl.PARAMS["FRAGMENT_FILES"]:
        print("Globbing fragment pattern: {}".format(pattern), flush=True)
        matched = glob.glob(pattern)
        print("  Found {} files".format(len(matched)), flush=True)
        frag_files += matched
    
    print("GET_DECODINGS={}, total frag_files={}".format(gl.PARAMS["GET_DECODINGS"], len(frag_files)), flush=True)
    ...
    if gl.PARAMS["GET_DECODINGS"] == 1:    # from fragmenting molecules
        ...
        print("Read {} fragment mols from files, replacing attachment points...".format(len(fragment_mols)), flush=True)
        fragment_mols = mol_utils.replace_attachment_points(fragment_mols)
        fragments = mol_utils.mols_to_frags(fragment_mols)
        print("Got {} unique fragments from files, merging with {} lead_frags...".format(len(fragments), len(lead_frags)), flush=True)
        fragments.update(lead_frags)
        print("Total fragments for encoding: {}".format(len(fragments)), flush=True)
        return get_encodings(fragments)
...
def encode_frags(frag_sets, encodings, max_frag):
    ...
    for i in range(X_mat.shape[0]):  # for every fragment set
        for j, f in enumerate(frag_sets[i]):  # for every fragment in set 
            # Fragments are always Mol objects (both DeepFMPO and BRICS)
...
def get_dist_matrix(fragments):
    ...
    if len(fragments) < 100000:
        def calc_similarity(i, j):
            return similarity(id_dict[i], id_dict[j], ms[i], ms[j])
    else:
        mfpgen = rdFingerprintGenerator.GetMorganGenerator(radius=2,fpSize=2048)
        print("Using fast similarity calculation as there are more than 1000 fragments.")
        def calc_similarity(i, j,mfpgen=mfpgen):
            fp1 = mfpgen.GetFingerprint(ms[i])
            fp2 = mfpgen.GetFingerprint(ms[j])
            eta=gl.PARAMS["ETA"]
            tau=0.6
            tau_fp=0.5
            return fast_similarity_with_prescreen(id_dict[i], id_dict[j], ms[i], ms[j], fp1, fp2, eta, tau, tau_fp)
    id_dict = {}  # dictionary {index: smi}
    ms = []       # molecules (as Mol objects)

    i = 0
    for smi, (frag_data, _) in fragments.items():
        # Handle both Mol objects and SMILES strings
        if isinstance(frag_data, str):
            # BRICS fragment stored as SMILES - convert to Mol
            mol = Chem.MolFromSmiles(frag_data)
        else:
            # DeepFMPO fragment stored as Mol object
            mol = frag_data
        ms.append(mol)
        id_dict[i] = smi
        i += 1
    ...
    print("Building tree from fragments ({})...".format(len(ms)), flush=True)
    print("  NUM_CORES={}, BATCHSIZE_D={}".format(gl.PARAMS["NUM_CORES"], gl.PARAMS["BATCHSIZE_D"]), flush=True)
    start = time.time()
    
    if gl.PARAMS["NUM_CORES"] == 0 or len(ms) < 2:
        print("  Using serial distance matrix computation", flush=True)
        for i in range(len(ms)):
            for j in range(i+1,len(ms)):
                distance_matrix[i,j] = calc_similarity(i, j)
                distance_matrix[j,i] = distance_matrix[i,j]        
    else:
        print("  Using parallel distance matrix computation with {} cores".format(gl.PARAMS["NUM_CORES"]), flush=True)
        i, j, j_max, i_max = 0, 1, 1, 0
        batch_count = 0
        num_batches = math.ceil(len(ms)*(len(ms)-1) / (2*gl.PARAMS["BATCHSIZE_D"]))
        print("  Total pairs: {}, num_batches: {}".format(len(ms)*(len(ms)-1)//2, num_batches), flush=True)
    ...
    print("Building distance matrix finished after {:.1f} seconds".format(time.time()-start), flush=True)
...
def get_hierarchy(fragments):
    ...
    print("Converting matrix finished after {:.1f} seconds".format(time.time()-start), flush=True)
...
def get_encodings_one_class(fragments):
    print('Getting hierarchy for {} fragments...'.format(len(fragments)), flush=True)
    pairings, id_dict = get_hierarchy(fragments)
    print('  Hierarchy done, building tree...', flush=True)
    t = build_tree_from_list(pairings, lookup=id_dict)
    print('  Tree built, encoding leafs...', flush=True)
    encodings = dict(t.encode_leafs())
    print('  Encodings done ({} entries), building decodings...'.format(len(encodings)), flush=True)
    
    # Fragments are always Mol objects (both DeepFMPO and BRICS)
    decodings = {}
    for k, v in encodings.items():
        decodings[v] = fragments[k][0]
